...
void UnderwaterDistort(inout vec2 texCoord) {
	...
	float wind = frameTimeCounter * 1.00;
	texCoord +=vec2(cos(texCoord.y * 32.0 + wind * 3.0),
	                sin(texCoord.x * 32.0 + wind * 1.7)) * 0.001 * 1.0;
...
void ColorSaturation(inout vec3 color) {
	...
	color = mix(color, mix(color, lightness, 1.0 - 1.20), sat);
	color = mix(color, lightness, (1.0 - lightness) * (2.0 - 1.20) / 2.0 * abs(1.20 - 1.0));
